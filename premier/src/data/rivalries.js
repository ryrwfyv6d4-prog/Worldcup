...
// A line for a real derby only. Every other fixture used to get a composed
// war-games paragraph from the two clubs' tiers, which read the same on every
// match and claimed points were "doubled" when the pricing does no such thing.
export function getMatchup(teamA, teamB, infoA, infoB) {
  const derby = getRivalry(teamA, teamB);
  return derby ? { ...derby, derby: true } : null;
}
