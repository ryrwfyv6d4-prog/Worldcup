...
import { SCORING, SCORING_LABELS, getFlag, GROUPS } from '../data/worldcup2026.js';
...
function getEpitaph(name, index) {
  return EPITAPHS[index % EPITAPHS.length](name);
}

function Graveyard({ assignments, drawType, fixtures, onSelectTeam }) {
  const fallen = useMemo(() => {
    ...
    for (const name of Object.keys(assignments)) {
      ...
      const bestStage = teamStatuses.reduce((best, ts) => {
        const order = ['GROUP_STAGE', 'LAST_32', 'LAST_16', 'QUARTER_FINALS', 'SEMI_FINALS'];
        const bi = order.indexOf(best);
        const ti = order.indexOf(ts.eliminatedAt);
  ...
  return (
    <div className="gy-section">
      <div className="gy-header">
        <div className="gy-cross">✠</div>
        <h3 className="gy-title">THE FALLEN</h3>
        <p className="gy-subtitle">Campaigns That Didn't Make It</p>
        <div className="gy-cross">✠</div>
      </div>
        ...
        {fallen.map((f) => (
          <div key={f.name} className="gy-tomb">
            ...
            <div className="gy-tomb-date">
              Fell in the {STAGE_LABELS_GY[f.bestStage] || f.bestStage}
            </div>
              ...
              {f.teams.map(({ team, eliminatedAt }) => (
                ...
                >
                  ...
                  <span className="gy-team-stage">
                    KIA: {STAGE_LABELS_GY[eliminatedAt] || '???'}
                  </span>
                </button>
              ))}
            </div>
